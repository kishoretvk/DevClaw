...
class CompressedAttention(torch.nn.Module):
    ...
    def forward(self,
        ...
        # Build kwargs dict to avoid positional/keyword conflict
        fwd_kwargs = {}
        if key is not None:
            fwd_kwargs['key'] = key
        if value is not None:
            fwd_kwargs['value'] = value
        ...
        return self.original_attention.forward(query, **fwd_kwargs)
    ...
    def compress_kv(self, kv_cache: Tuple[torch.Tensor, torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor]:
        """
